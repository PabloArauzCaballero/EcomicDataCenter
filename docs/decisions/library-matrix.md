...
| Adapter HTTP | Express, Fastify | Fastify 5.10.0 | Nest platform-fastify 11 | Plugins seleccionados deben soportar Fastify 5 | MIT | Menor overhead esperado; benchmark pendiente | Express adapter con ADR y pruebas | ADR-0001 |
| Assets de Swagger local | `@fastify/static`, contrato exportado sin UI | `@fastify/static` 9.1.3 | Compatible con Fastify 5 y requerido por el adapter para servir Swagger UI | Swagger está prohibido en producción; el plugin solo respalda documentación local habilitada explícitamente | MIT | Fuera del hot path productivo | Mantener solo exportación OpenAPI y desactivar UI | Evidencia del arranque local + integración Nest Fastify |
