...
await ask(
  'vistas prensa',
  "select count(*)::text from information_schema.views where table_schema='read_models' and table_name like 'press%'",
);
await ask(
  'notas',
...
await ask(
  'ultima nota',
  "select coalesce(max(event_date)::text,'-') from read_models.press_article",
);
await ask(
  'con acento',
  "select count(*)::text from read_models.press_article where headline ~ '[aeiouAEIOU]'",
);
await ask(
  'otros pct',
  "select round(100.0*count(*) filter (where topic='OTROS')/greatest(count(*),1),1)::text from read_models.press_article where status='PUBLISHED' and not superseded",
);
