...
await ask('vista press_article', "select count(*)::text from read_models.press_article where status='PUBLISHED' and not superseded");
await ask('matviews', "select matviewname from pg_matviews where schemaname='read_models'");
await ask('migracion 0053', "select name from infrastructure.migration_history where name like '0053%'");
await client.end();
