...
| 3.2  | Bonos soberanos                     |   🟡   | `read_models.sovereign_yield_curve` (ADR 0021), `raw_observation` con `recordType = YIELD_CURVE_POINT`                                 | Curva doméstica de la BBV cubierta por emisor, instrumento, moneda, lado y banda de plazo, sin historia previa al colector. Falta `financial_instrument` con ISIN, cupón, vencimiento y YTM por título; faltan las emisiones internacionales (OTC) y el riesgo país EMBI, que no tiene fuente pública legible por máquina                     |
...
| 3.4  | Situación socioeconómica            |   🟡   | `geographic_unit`, `indicator`, `series`, sectores `SOCIAL` (32 series) e `INSTITUCIONAL` (12) de `read_models.macro_indicator_annual` | Cobertura nacional cargada: mortalidad, escolarización, agua, saneamiento, electricidad, pobreza, reparto del ingreso, homicidios, IDH y gobernanza. Sigue sin desagregación subnacional ni grupo poblacional o nivel de agregación como dimensiones estándar sembradas; la libertad económica como índice (Heritage, Fraser) no es obtenible |
