...
/**
 * Files the political coverage, which is most of what «Otros» still held.
 *
 * `gobierno` alone appeared four hundred and eighteen times in the residual
 * bucket. In Bolivia the state is an economic actor in nearly every sector, so
 * its coverage was never going to sit in a subject list built out of
 * commodities and instruments — it needed one of its own, and a reader asking
 * "what is the government doing" now has somewhere to click.
 *
 * It is filed second-to-last, ahead only of crime reporting and the residual,
 * and that order is what keeps it honest: a ministerial announcement about
 * diésel is claimed by fuel, a decree on wages by labour, a budget by fiscal.
 * Only coverage that is about the politics itself lands here.
 *
 * Three subjects grow at the same time with what the count showed: telecoms,
 * technology and aviation are industries and belong to the real economy; the
 * pandemic is a social subject; and «negocios», «económico» and «ingresos» are
 * the economy discussed as itself.
 */
