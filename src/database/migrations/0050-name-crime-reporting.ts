...
/**
 * Names crime reporting for what it is instead of leaving it under «Otros».
 *
 * These outlets carry the whole newsroom, not only the business desk, so a
 * murder in Guayaramerín reaches the same feed as a fuel decree. Filing it as
 * "other subjects" told the reader the observatory could not classify it, when
 * in fact it is perfectly classifiable and simply not economic — and it padded
 * the residual bucket the reader is asked to trust as small.
 *
 * It is filed LAST, after every economic subject, and that order is the whole
 * safeguard: "instructive classifying coronavirus as a workplace accident" is a
 * labour story that happens to contain an accident, and it is claimed by labour
 * before this rule ever sees it. Only what no economic rule wanted, and does
 * read as crime reporting, lands here.
 *
 * `robo`, `droga`, `policía`, `accidente`, `muerte` and `detenido` were tried
 * and left out: contraband seizures, police operations at the border and
 * industrial accidents are economic coverage, and those words drag them out of
 * the subjects where they belong.
 */
