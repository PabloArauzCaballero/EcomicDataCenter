...
/**
 * Files the last of the actions the tone lexicon was not claiming.
 *
 * Ranking what remained unmarked showed the honest limit: it is almost all
 * subject nouns — "Bolivia exportó X toneladas a Brasil" — coverage that
 * carries no tonal word at all, and forcing one on it would be invention. Only
 * about a tenth of it was a verb the list had missed, and every one of those is
 * an action taken: delivered, bought, allocated, invested, guaranteed.
 *
 * A statement — "asegura", "afirma", "sostiene" — was considered and left out.
 * It is attribution, not tone, and adding it would have made the unmarked share
 * look smaller by redefining the axis rather than by reading more of the
 * corpus. What is left unmarked stays unmarked, and the panel says why.
 */
