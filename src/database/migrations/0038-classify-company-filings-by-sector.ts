...
/**
 * Files each material event under the industry of the company that filed it.
 *
 * The exchange does not publish an industry taxonomy for its issuers, so this
 * is derived rather than quoted, and the derivation is kept here in one visible
 * expression instead of being baked into the data. Each rule matches the
 * issuer's own registered name — a company called "Empresa Minera San Lucas
 * S.A." is in mining because it says so — and anything no rule matches stays
 * OTROS rather than being guessed into a bucket.
 *
 * Order is load-bearing. "Mercantil Santa Cruz Agencia de Bolsa S.A." is a
 * broker, not a bank, so the securities rules are tested before the banking
 * ones; "Distribuidora de Electricidad La Paz" is a utility, not a retailer,
 * so electricity is tested before commerce. Each block is therefore narrower
 * than the one below it.
 *
 * The view also exposes the issuer's code and the exchange's filing identifier,
 * which the register carries and the earlier capture did not.
 *
