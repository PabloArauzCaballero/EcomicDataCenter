...
/**
 * Reporting model for press coverage of the Bolivian economy.
 *
 * Kept apart from every indicator model on purpose. A table states a
 * measurement and an article reports one, and the observatory's whole claim to
 * being checkable rests on a reader always knowing which they are looking at.
 * Nothing here can reach a series: this view reads only PRESS_COVERAGE
 * observations, and the tier that admits a figure into a series admits only
 * OFFICIAL publishers.
 *
 * The topic is derived here rather than stored, like the industry of a filing.
 * A general feed carries a fuel decree beside a film festival, so the question
 * is which coverage bears on the economy at all — and a derivation that lives
 * in one visible expression can be argued with and corrected, where a label
 * baked into the data at capture time cannot.
 *
 * Order is load-bearing. A story about the diesel decree's effect on prices is
 * filed under fuel, because that is what it is about; the narrower subject wins
 * over the broader one, so the specific blocks are tested first and OTROS
 * catches what no rule claims rather than guessing.
 */
...
WITH published AS (
  SELECT
    ...
    ro.payload_json ->> 'storageUri'      AS listing_url,
