...
/**
 * Widens the vocabularies that file press coverage by subject and by tone.
 *
 * Two thirds of the corpus was landing in «Otros» and seven tenths carried no
 * tone mark. That is not a country whose coverage is mostly uncategorisable —
 * it is a word list that was too short. Coverage of the IMF, of poverty, of
 * soya exports, of a hydroelectric plant, of a blockade over royalties and of
 * a transport strike all fell through it.
 *
 * What is added is vocabulary, not judgement. The rules stay what they were:
 * lists of words matched against the headline and the standfirst, which a
 * reader can print out and argue with. Three subjects are new because the
 * coverage plainly has them and there was nowhere to put them — electricity,
 * public works and the social accounts — and everything the lists still do not
 * claim stays under «Otros» rather than being filed somewhere flattering.
 *
 * Order is load-bearing and unchanged. Alarm and conflict outrank direction,
 * because a story about a blockade that also mentions prices is about the
 * blockade. Fuel outranks the rest, because in Bolivia it is the subject that
 * drives the others. NEUTRO remains what no rule claimed, never a reading of
 * calm.
 *
 * `demanda` is deliberately absent from the conflict list: in an economic
 * corpus it is far more often demand than a lawsuit.
 */

const dropView = `DROP VIEW IF EXISTS read_models.press_term_mention;
DROP VIEW IF EXISTS read_models.press_article;`;
...
SELECT
  ...
  CASE
    ...
    WHEN subject ILIKE ANY (ARRAY[
      ...
      '%sistema financiero%', '%banca%', '%fmi%', '%fondo monetario%',
    ...
    WHEN subject ILIKE ANY (ARRAY[
      ...
      '%expocruz%', '%puerto%', '%bioceánic%', '%bioceanic%'
    ...
    WHEN subject ILIKE ANY (ARRAY[
      ...
      '%renta dignidad%', '%despido%', '%contratac%'
    ]) THEN 'LABORAL'
    WHEN subject ILIKE ANY (ARRAY[
    ...
    WHEN subject ILIKE ANY (ARRAY[
      ...
      '%competitividad%', '%la economía%', '%la economia%', '%recesión%',
      '%recesion%', '%rueda de negocios%', '%clima de negocios%'
    ]) THEN 'ACTIVIDAD'
    WHEN subject ILIKE ANY (ARRAY[
      '%producción%', '%produccion%', '%industria%', '%agro%', '%soya%',
      '%minería%', '%mineria%', '%minero%', '%maíz%', '%maiz%', '%sorgo%',
